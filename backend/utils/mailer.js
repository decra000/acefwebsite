...
class MailerService {
  ...
  async createTransporter(accountKey = 'info') {
    try {
      ...
      console.log(`📧 Creating email transporter for account: ${accountKey}`);

      // Get account configuration from database
      const account = await emailAccountModel.getEmailAccount(accountKey);
      
      if (!account) {
        console.warn(`⚠️ Account '${accountKey}' not found, using fallback`);
        const fallbackAccount = await emailAccountModel.getFallbackAccount();
      ...
      console.error(`❌ Error creating transporter for ${accountKey}:`, error);
      throw error;
    }
  }

  // Get sender info for email headers
  async getSenderInfo(accountKey = 'info') {
    try {
      const account = await emailAccountModel.getEmailAccount(accountKey);
      ...
      if (!account) {
        const fallbackAccount = await emailAccountModel.getFallbackAccount();
        if (fallbackAccount) {
          return `"${fallbackAccount.from_name}" <${fallbackAccount.smtp_user}>`;
        }
        return '"ACEF Team" <info@acef-ngo.org>'; // Hardcoded fallback
      }
      
      return `"${account.from_name}" <${account.smtp_user}>`;
    } catch (error) {
      console.error('Error getting sender info:', error);
      return '"ACEF Team" <info@acef-ngo.org>'; // Fallback
    }
  }

  // Clear transporter cache
  clearCache(accountKey = null) {
    if (accountKey) {
      this.transporterCache.delete(accountKey);
      console.log(`🧹 Cleared cache for ${accountKey}`);
    } else {
      this.transporterCache.clear();
      console.log('🧹 Cleared all email transporter cache');
    }
  }

  // Test email account connection
  async testEmailConnection(accountKey = 'info') {
    try {
      console.log(`🔍 Testing email connection for: ${accountKey}`);
      
      // Validate account configuration first
      ...
      console.log(`✅ Email connection test successful for: ${accountKey}`);
      
      return {
        success: true,
        message: `Email connection working for ${accountKey}`,
        account: validation.account
      };

    } catch (error) {
      console.error(`❌ Email connection test failed for ${accountKey}:`, error);
  ...
  // Handle email errors with appropriate messaging
  handleEmailError(error) {
    if (error.code === 'EAUTH') {
      return new Error('Email authentication failed. Check your email credentials and password.');
    } else if (error.code === 'ECONNECTION') {
      return new Error('Could not connect to email server. Check your internet connection.');
    } else if (error.message.includes('Invalid login')) {
      return new Error('Invalid email credentials. Please verify the email configuration.');
    } else {
    ...
    try {
      ...
      const mailOptions = {
        ...
        html: `
          ...
          <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
            <div style="background: #f8f9fa; padding: 30px; border-radius: 10px; border: 1px solid #dee2e6;">
              <h2 style="color: #007bff; margin-bottom: 20px;">ACEF Communication</h2>
              ...
              <div style="margin-top: 30px; text-align: center; color: #6c757d; font-size: 14px;">
                <p>Best regards,<br/><strong>ACEF Admin Team</strong></p>
              </div>
            </div>
          </body>
          </html>
  ...
  async sendDonationBadgeEmail({
    ...
    try {
      ...
      // Validate badge buffer with comprehensive checks
      if (!badgeBuffer) {
          ...
          <body style="font-family: 'Arial', sans-serif; line-height: 1.6; color: #333; max-width: 650px; margin: 0 auto; padding: 20px; background: #f8fafb;">
            ...
            <div style="background: white; padding: 35px; border-radius: 15px; margin-bottom: 25px; box-shadow: 0 5px 20px rgba(0,0,0,0.08); border-left: 5px solid #16a34a;">
              ...
              <div style="background: linear-gradient(135deg, #ecfdf5 0%, #f0fdf4 100%); padding: 25px; border-radius: 12px; margin: 25px 0; border: 1px solid #bbf7d0;">
                <h3 style="color: #065f46; margin: 0 0 15px 0; font-size: 1.3rem;">🌱 Your Impact</h3>
    ...
    try {
      ...
      const reminderContent = {
        ...
        thank_you_follow: {
          ...
          urgency: 'Together, we\'re building a more sustainable future for Africa.'
        }
      };
  ...
  async sendJobApplicationEmail({ to, subject, applicantName, jobTitle, message }) {
    try {
  ...
  async sendWelcomeEmail(to, unsubscribeLink) {
    try {
  ...
  async testAllConnections() {
    try {
      console.log('🔍 Testing all ACEF email connections...');
      ...
      for (const account of accounts) {
        try {
          const result = await this.testEmailConnection(account.account_key);
          results.push({
      ...
      console.log(`✅ Email connection tests completed: ${successful}/${total} successful`);
      
      return {
        success: successful === total,
        message: `${successful}/${total} email accounts working`,
        results,
        summary: {
          total,
          successful,
      ...
      console.error('❌ Email connection tests failed:', error.message);
      ...
      const mailOptions = {
        ...
        html: `
          ...
          <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 650px; margin: 0 auto; padding: 20px; background: #f8fafb;">
            ...
            <!-- Header -->
            <div style="background: linear-gradient(135deg, #10b981 0%, #047857 100%); padding: 30px; border-radius: 15px; text-align: center; margin-bottom: 30px;">
              <h1 style="color: white; margin: 0; font-size: 1.8rem;">ACEF Newsletter</h1>
              <p style="color: rgba(255, 255, 255, 0.9); margin: 5px 0 0 0; font-size: 1rem;">${subject}</p>
            </div>
            
            <!-- Content -->
            <div style="background: white; padding: 35px; border-radius: 15px; margin-bottom: 25px; box-shadow: 0 5px 20px rgba(0,0,0,0.08);">
              ${recipientName ? `<p style="font-size: 1.1rem; margin-bottom: 20px;">Hello ${recipientName},</p>` : ''}
              ${htmlContent}
            </div>
            ...
            <div style="text-align: center; padding-top: 30px; border-top: 2px solid #e5e7eb; color: #6b7280; font-size: 0.9rem;">
              <p style="margin: 0 0 10px 0;">
                <strong style="color: #047857;">African Climate and Environment Foundation (ACEF)</strong>
              </p>
              <p style="margin: 0 0 15px 0;">
                Building climate resilience across Africa, together.
              </p>
              <p style="margin: 0; font-size: 0.8rem;">
                ...
                <a href="${unsubscribeLink}" style="color: #047857;">Unsubscribe</a>
              </p>
            </div>
          </body>
          </html>
